...
def listUser():
    try:
        ...
        if startat <= per_page:
            startat = 0
        count = exeReq("SELECT count(*) FROM user WHERE grp != 'deleted';")
        ...
        # Get data
        list = exeReq("SELECT login, email, grp FROM user WHERE grp != 'deleted' LIMIT {}, {};".format(startat, per_page))

        wEvent('/html/v1.0/user/list','exeReq','Get list','OK')
        return render_template('listUser.html', list = list, maps = getMaps(), pagination=pagination)
    except Exception as e:
        wEvent('/html/v1.0/user/list','exeReq','Get list','KO')
        return 'List error'
